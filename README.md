...
> ⚠️ Warning: Forked from `givebutter/laravel-custom-fields` for personal use.

## Installation

To get started, add the `dnl4/laravel-custom-fields` package to your `composer.json` file and update your dependencies:

```bash
composer require dnl4/laravel-custom-fields:*
```

Publish the migration:
```bash
php artisan vendor:publish --provider="Dnl4\LaravelCustomFields\LaravelCustomFieldsServiceProvider" --tag="migrations"
```
